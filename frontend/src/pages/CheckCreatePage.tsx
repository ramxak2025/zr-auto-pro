...
function ClientSearch({ onSelect, selectedClient, onClear, onAddNew }: {
  onSelect: (client: Client) => void; selectedClient: Client | null; onClear: () => void; onAddNew: () => void;
}) {
    ...
    <div className="relative">
      <div className="flex gap-2">
        ...
        <button type="button" onClick={onAddNew}
          className="flex h-[42px] w-[42px] items-center justify-center rounded-xl border border-blue-200 bg-blue-50 text-blue-600 hover:bg-blue-100 active:scale-95 transition-all flex-shrink-0"
...
function AddClientModal({ onClose, onClientCreated }: {
  onClose: () => void;
  ...
  const [duplicateInfo, setDuplicateInfo] = useState<{ type: 'phone' | 'plate'; client: Client } | null>(null);
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const clientsRes = await clientsApi.getAll({ search: phone.trim(), limit: 50 });
      const clientsList = (clientsRes.data as any).data as Client[];
      const byPhone = clientsList.find((c) => c.phone === phone.trim());
      if (byPhone) {
        setDuplicateInfo({ type: 'phone', client: byPhone });
        setIsLoading(false);
        return;
      }

      // Check plate duplicate
      if (plateNumber.trim()) {
        const carsRes = await carsApi.getAll({ search: plateNumber.trim(), limit: 50 });
        const carsList = (carsRes.data as any).data as Car[];
        const byPlate = carsList.find(
          (c) => c.plateNumber.toUpperCase() === plateNumber.trim().toUpperCase(),
        );
        if (byPlate) {
          const ownerRes = await clientsApi.getById(byPlate.clientId);
          setDuplicateInfo({ type: 'plate', client: ownerRes.data as any as Client });
          setIsLoading(false);
          return;
        }
      }
      ...
      // Create client
      const clientRes = await clientsApi.create({ fullName: fullName.trim(), phone: phone.trim() });
      let newClient: Client = clientRes.data as any as Client;

      // Create car if provided
      if (plateNumber.trim() && makeModel.trim()) {
        ...
        newClient = { ...newClient, cars: [(carRes.data as any) as Car] };
      } else {
        newClient = { ...newClient, cars: [] };
      }
  ...
  const inputClass = 'block w-full rounded-xl border border-gray-200 bg-gray-50/50 px-3.5 py-2.5 text-sm placeholder-gray-400 focus:border-blue-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/10';

  return (
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center">
      <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={!isLoading ? onClose : undefined} />
      <div className="relative w-full md:max-w-md bg-white rounded-t-3xl md:rounded-2xl overflow-hidden max-h-[90vh] overflow-y-auto">
        <div className="flex justify-center pt-3 pb-1 md:hidden">
          <div className="w-10 h-1 bg-gray-200 rounded-full" />
        </div>
        <div className="px-6 pt-4 pb-3">
          <h3 className="text-lg font-bold text-gray-900">Новый клиент</h3>
          <p className="text-xs text-gray-400 mt-0.5">Заполните данные клиента</p>
        </div>
        <div className="px-6 pb-6 space-y-3">
          ...
          <div>
            ...
            <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)}
              placeholder="+7 (900) 123-45-67" className={inputClass} />
          </div>

          <div className="border-t border-gray-100 pt-3">
            <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-2">Автомобиль (необязательно)</p>
            <div className="grid grid-cols-2 gap-2.5">
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Госномер</label>
                <input type="text" value={plateNumber} onChange={(e) => setPlateNumber(e.target.value.toUpperCase())}
                  placeholder="А123БВ777" className={inputClass} />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Марка / Модель</label>
                <input type="text" value={makeModel} onChange={(e) => setMakeModel(e.target.value)}
                  placeholder="Toyota Camry" className={inputClass} />
              </div>
            </div>
          </div>
          ...
          {duplicateInfo && (
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-3.5">
              <p className="text-sm font-semibold text-amber-800">
                {duplicateInfo.type === 'phone' ? 'Клиент уже добавлен' : 'Автомобиль уже зарегистрирован'}
              </p>
              <p className="text-xs text-amber-600 mt-0.5">
                {duplicateInfo.client.fullName} — {duplicateInfo.client.phone}
              </p>
              {duplicateInfo.client.cars && duplicateInfo.client.cars.length > 0 && (
                <p className="text-[11px] text-amber-500 mt-0.5">
                  {duplicateInfo.client.cars.map((c) => `${c.plateNumber} ${c.makeModel}`).join(', ')}
                </p>
              )}
              <button type="button" onClick={() => handleSelectExisting(duplicateInfo.client)}
                className="mt-2.5 flex items-center gap-1.5 rounded-lg bg-blue-100 px-3 py-1.5 text-xs font-semibold text-blue-700 hover:bg-blue-200 transition-colors">
                <UserIcon className="h-3 w-3" />Выбрать этого клиента
              </button>
            </div>
          )}
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="button" onClick={handleSubmit} disabled={isLoading}
              className="flex-[1.5] flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-bold text-white hover:bg-blue-700 disabled:opacity-50 active:scale-[0.98] transition-all">
    ...
    <>
      ...
      {showAddClientModal && (
        <AddClientModal
          ...
          onClientCreated={(client) => { setSelectedClient(client); setShowAddClientModal(false); }}
        />
      )}
    </>
  );
