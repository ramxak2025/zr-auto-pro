...
export interface PointsListResponse {
  points: TenantPoint[];
  /**
   * Текущая точка запрашивающего. Сервер может ПОДСТАВИТЬ её сам: у
   * сотрудника без права user_management филиал обязателен, и если доступна
   * ровно одна точка, GET /points закрепляет её за пользователем (160).
   * null остаётся только у владельца/админа в режиме «Все точки» и у
   * одноточечных тенантов.
   */
  currentPointId: string | null;
}
...
export interface PointSummary {
  ...
  revenueMonth: number;
  /**
   * ЧИСТАЯ прибыль филиала с начала месяца — валовая прибыль МИНУС расходы
   * ЭТОГО филиала (161), тем же правилом, что и «чистая прибыль» на главной.
   * Держателю profit_view — реальная (гарантия входит убытком); без права —
   * всегда 0, как и в остальных ответах API.
   *
   * ЧЕГО ЗДЕСЬ НЕТ: плановая постоянка (fixed_costs, employee_compensation)
   * точки не имеет и в филиальный расчёт не входит — она общетенантная.
   * Поэтому сумма profitMonth по филиалам может быть ВЫШЕ сетевой чистой
   * прибыли ровно на эту постоянку. Разнести её по филиалам без решения
   * владельца («заводим на каждый филиал» или «делим по ключу») нельзя —
   * любой автоматический дележ был бы выдуманной цифрой.
   */
  profitMonth: number;
  checksToday: number;
  checksMonth: number;
