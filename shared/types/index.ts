...
export interface Tenant {
   ...
   * по этому поясу сервер считает ВСЕ бизнес-сутки: журнал, касса, зарплата,
   * смены, отчёты, напоминания. Мутируется через PATCH /my-company
   * (company_manage), значение проверяется сервером по белому списку
   * российских поясов. Absent на легаси-payload'ах → считать 'Europe/Moscow'
   * (см. DEFAULT_TIMEZONE в shared/utils/formatters).
   *
   * Приезжает не только с GET /my-company, но и внутри `User.tenant` из
   * /auth/login и /auth/me — ЛЮБОЙ роли: /my-company закрыт ключом
   * company_manage, и мастер иначе остался бы без пояса. Клиенты берут его
   * оттуда (useTenantTimezone на mobile и web).
   */
  timezone?: string;
  /**
 ...
 * Сотрудник.
 *
 * ВНИМАНИЕ: В СПИСКАХ СОСТАВ ПОЛЕЙ ЗАВИСИТ ОТ ПРАВ СМОТРЯЩЕГО (аудит 2026-09).
 * Роуты GET /users, /users/masters и /users/:id открыты всем аутентифицированным
 * — ими питаются пикер мастера в Кассе, график и резолв имён в журнале,
 * расходах и зарплате. Поэтому ПДн и деньги сотрудника уезжают только
 * держателю права 'user_management' и самому сотруднику про себя, а всем
 * остальным эти ключи ОТСУТСТВУЮТ в ответе:
 *
 *     username, phone, salaryPercent, productSalaryPercent,
 *     permissions, canAddExpenses, dailyExpenseLimit
 *
 * Типы у них не сделаны опциональными сознательно: все существующие экраны,
 * которые их читают, либо закрыты тем же правом, либо читают через `|| 0` /
 * `|| '—'`, и расширение типа заставило бы переписать их без пользы. Новый код,
 * который читает эти поля из СПИСКА (а не из /auth/me своего пользователя),
 * обязан переживать их отсутствие.
 *
 * /auth/login, /auth/select-point и /auth/me всегда отдают полный состав: там
 * речь о самом себе.
 */
export interface User {
  ...
  roleName?: string | null;
  /**
   * ФИЛИАЛ ТЕКУЩЕЙ СЕССИИ (163). Приезжает в /auth/login, /auth/select-point и
   * /auth/me и означает «в каком филиале работает ЭТОТ токен», а не «какой
   * филиал человек выбрал в последний раз на любом устройстве»: веб и телефон
   * одного сотрудника могут одновременно работать в разных филиалах.
   *
   * null = у тенанта нет ни одного живого филиала (одноточечный автосервис).
   * Режима «все филиалы» больше не существует: сменить филиал можно только
   * выходом и новым входом (POST /points/switch лишь запоминает филиал для
   * следующего входа).
   *
   * absent в путях, которые филиал не отдают (списки сотрудников и т.п.).
   */
...
/**
 * 161 — тот же 409, но когда номер занят карточкой ДРУГОГО ФИЛИАЛА
 * (tenants.pointsSharedClients = false).
 *
 * Уникальный индекс телефона построен по ТЕНАНТУ и точки не знает — это
 * сознательно: разрешить один номер на нескольких филиалах значит завести двух
 * «одинаковых» клиентов, чьи долги, бонусы и рассрочки разъедутся по разным
 * карточкам. Поэтому ограничение остаётся, а меняется ОТВЕТ: ни имени, ни
 * `clientId` — их владелец видеть не должен, а «Перейти к клиенту» всё равно
 * привело бы в 404. Клиенту достаточно показать `message`: он объясняет, что
 * делать (попросить владельца перевести клиента или включить общую базу).
 *
 * Совместимость: старые сборки проверяют `code === 'CLIENT_PHONE_EXISTS' &&
 * clientId` — обе проверки не проходят, и они честно показывают текст ошибки.
 */
export interface ClientPhoneConflictOtherPoint {
  message: string;
 ...
 * лимит; DELETE = архив. Тенант читает свои филиалы через GET /points.
 * 0–1 филиал = одноточечный режим, клиент ничего про филиалы не показывает.
 *
 * 163 — ФИЛИАЛ ВЫБИРАЕТСЯ ПРИ ВХОДЕ и живёт в сессии. Переключения внутри
 * приложения нет (POST /points/switch отвечает 409); доступ сотрудника к
 * филиалам настраивается в его карточке — GET/PUT /users/:id/points
 * (user_management), а раздел «Филиалы» показывает состав для просмотра.
 */
...
/**
 * GET /points — живые филиалы своего тенанта + филиал ТЕКУЩЕЙ СЕССИИ.
...
export interface PointsListResponse {
  points: TenantPoint[];
  /**
   * Филиал ТЕКУЩЕЙ СЕССИИ — тот, что лежит в токене (163). Ручка ничего не
   * подставляет и ничего не пишет: филиал выдан при входе. null = у тенанта
   * нет живых филиалов (одноточечный автосервис).
   */
  currentPointId: string | null;
}

/**
 * GET /users/:id/points — на каких филиалах может работать сотрудник
 * (user_management). ПУСТОЙ массив = НЕ ОГРАНИЧЕН: сотруднику доступны все
 * живые филиалы тенанта. Это конвенция внедрения (156), а не «доступов нет»:
 * тенант, который никого никуда не назначал, продолжает работать без единой
 * настройки. Тот же тип — ответ PUT /users/:id/points.
 */
...
/**
 * Карточка филиала в разделе «Филиалы» (GET /points/summary, миграции
 * 156/160). Деньги считаются ТЕМИ ЖЕ правилами, что на главной и в
 * dashboard-v2: гарантия исключена из выручки и заменена убытком в прибыли,
 * учитываются только проведённые живые чеки, границы дня и месяца — в поясе
 * тенанта (tenants.timezone).
 */
export interface PointSummary {
  ...
  isMain: boolean;
  /**
   * ФИЛИАЛ ЗАКРЫТ (архив, tenant_points.is_active = false).
   *
   * Такая карточка приезжает ТОЛЬКО когда у закрытого филиала есть деньги или
   * работа в показанном периоде. Раньше сводка брала лишь действующие филиалы,
   * а деньги закрытого оставались в итогах тенанта: владелец складывал
   * карточки, не получал цифру с главной и читал это как пропажу. Теперь сумма
   * карточек сходится с итогом сети.
   *
   * ЧТО ОБЯЗАН СДЕЛАТЬ КЛИЕНТ: подписать карточку «Закрыт» и НЕ предлагать в
   * неё войти (сервер туда и не пустит — филиал заархивирован). Молча
   * показывать её наравне с живыми нельзя: владелец пойдёт искать в закрытом
   * филиале сегодняшнюю работу.
   */
  isArchived: boolean;
  ...
  revenueMonth: number;
  /**
   * ЧИСТАЯ прибыль филиала с начала месяца — ДОСЛОВНО netProfitMonth главной:
   * прибыль по чекам МИНУС расходы ЭТОГО филиала (161) МИНУС премии деньгами
   * и мотивация этого филиала за тот же месяц. Держателю profit_view —
   * реальная (гарантия входит убытком); без права — всегда 0, как и в
   * остальных ответах API.
   *
   * ЧЕГО ЗДЕСЬ НЕТ: плановая постоянка (fixed_costs, employee_compensation)
   * точки не имеет и в филиальный расчёт не входит — она общетенантная. Но её
   * нет и в netProfitMonth на главной, поэтому расхождения между экранами это
   * не создаёт: постоянка живёт в ДРУГОЙ метрике дашборда (netProfitAccrual).
   * Разнести её по филиалам без решения владельца («заводим на каждый филиал»
   * или «делим по ключу») нельзя — любой автоматический дележ был бы
   * выдуманной цифрой.
   */
  profitMonth: number;
  checksToday: number;
  checksMonth: number;
  /**
   * Сколько мастеров/админов филиала сейчас в открытой смене (161 — у смен
   * появился point_id, считается по ним).
   *
   * null = учёт смен у тенанта ВЫКЛЮЧЕН (tenants.shifts_enabled, дефолт
   * false): источника факта нет, и карточка обязана показать прочерк.
   * 0 = учёт включён, но сегодня действительно никто не открыл смену.
   * Клиенту РАЗЛИЧАТЬ эти два случая обязательно — «0» вместо прочерка
   * читается как «филиал не работает».
   */
  mastersOnShift: number | null;
}

/**
 * GET /points/summary — сводка по точкам тенанта.
 *
 * СОСТАВ: все живые точки + закрытые (isArchived), у которых в показанном
 * периоде есть деньги или работа. Закрытая точка с нулями не приезжает —
 * складывать в ней нечего.
 *
 * ПОРЯДОК: живые первыми (основной сервис во главе, дальше как в GET /points),
 * закрытые — в хвосте. Клиент, который рисует карточки подряд, получает их
 * ровно в том порядке, в каком владелец складывает суммы.
 */
...
export interface CashShift {
   ...
   * 161 — размен переносится ВНУТРИ филиала: деньги ящика точки А не могут
   * стать разменом точки Б.
   */
  carryoverAmount?: number | null;
  /**
   * 161 — филиал, на котором смену открыли. Кассовая смена своя у каждого
   * филиала: Z-отчёт считает чеки и расходы ТОЛЬКО этой точки, а «одна
   * открытая смена» действует на филиал, а не на тенант. null = одноточечный
   * тенант (поведение прежнее).
   *
   * Открыть смену без филиала, когда точки у тенанта заведены, сервер не
   * даёт (400 «Выберите филиал…»): «ничья» смена посчитала бы чеки всей сети
   * и задвоила бы их с Z-отчётом филиала, где они были пробиты.
   */
  pointId?: string | null;
}
 ...
 *
 *   expectedAmount = openingAmount + cashSales + installmentCash
 *                    − cashExpenses − cashReturns − collectionsTotal
...
export interface CashShiftReport {
  ...
  cashExpenses: number;
  /**
   * 164 — наличная часть возвратов, ОФОРМЛЕННЫХ в окне смены по чекам ПРОШЛЫХ
   * окон (возврат в день продажи уже вычтен внутри cashSales). Деньги выданы
   * из ящика сегодня, поэтому уменьшают expectedAmount сегодняшней смены.
   * Absent на старом бэкенде.
   */
  cashReturns?: number;
  /**
   * 164 — погашения рассрочки, принятые НАЛИЧНЫМИ в окне смены. Чек-источник
   * датирован днём продажи, поэтому в cashSales их нет — но деньги физически
   * легли в ящик и увеличивают expectedAmount. Absent на старом бэкенде.
   */
...
export interface FinancialReport {
  ...
  warrantyLoss?: number;
  /**
   * Премии деньгами (salary_premiums.type='cash') за период. Третий вид
   * зарплатного начисления рядом с чековым (`salaries`) и выплатным (расход
   * категории «Зарплата», исключён из прибыли по имени): реальные деньги
   * мастеру, которых нет ни там, ни там. УЖЕ вычтены из netProfit — отдаются
   * отдельно, чтобы UI мог показать строку. Optional: старый бэкенд = 0.
   */
  premiums?: number;
  /**
   * «Мотивация» (motivation_accruals) за период — бонус мастера за товары
   * акции. Как и premiums, УЖЕ вычтена из netProfit. Optional: старый бэкенд = 0.
   */
  motivation?: number;
  grossProfit: number;
  netProfit: number;
...
/** Одна точка оси графика дашборда: день (week/month), час (today) или месяц (year). */
export interface DashboardChartPoint {
  /** 'YYYY-MM-DD' для day/month-осей, ISO-инстант для почасовой оси 'today'. */
  date: string;
  revenue: number;
  profit: number;
  checkCount: number;
...
/**
 * Итоги ПРОШЛОГО периода для дельты на дашборде — считает сервер, в поясе
 * автосервиса.
 *
 * ПОЧЕМУ НЕ КЛИЕНТ. Раньше мобилка тянула второй запрос графика (month, −1) и
 * сравнивала месяц-к-дате (1–9 сентября) с ПОЛНЫМ прошлым месяцем (1–31
 * августа) — в начале месяца дельта всегда была сильно отрицательной и ничего
 * не значила. Теперь сервер обрезает прошлый месяц по тому же дню («на ту же
 * дату») и присылает готовую подпись — один расчёт на iOS, Android и web.
 *
 * Неделя, день и год сравниваются равными отрезками (прошлая неделя / вчера /
 * прошлый год целиком) — их семантика не менялась.
 */
export interface DashboardChartComparison {
  totalRevenue: number;
  totalProfit: number;
  totalChecks: number;
  /** Местная календарная дата начала окна сравнения, 'YYYY-MM-DD'. */
  from: string;
  /** Местная календарная дата конца окна ВКЛЮЧИТЕЛЬНО, 'YYYY-MM-DD'. */
  to: string;
  /** true — прошлый месяц обрезан по дню («1–9 августа»), а не взят целиком. */
  truncated: boolean;
  /** Готовая подпись чипа: «к 9 августа», «к августу», «к предыдущей неделе». */
  label: string;
}
...
export interface DashboardChartResponse {
  ...
  totalChecks: number;
  /**
   * null — сравнивать не с чем (окно в будущем). Поле ОПЦИОНАЛЬНО: старый
   * бэкенд его не шлёт, клиент обязан пережить отсутствие и показать прочерк.
   */
  previous?: DashboardChartComparison | null;
}
...
export interface Shift {
  ...
  note?: string;
  /**
   * 161 — филиал, В КОТОРОМ смена была ОТКРЫТА (штампуется сервером из текущей
   * точки открывающего). Переключение филиала в середине дня смену НЕ
   * переносит: человек физически отработал там, где начал. null =
   * одноточечный тенант либо смена, открытая в режиме «Все точки».
   */
  pointId?: string | null;
}
...
export interface NetProfitAccrual {
  ...
  mtd: {
    ...
    /**
     * Премии деньгами + «мотивация», начисленные мастерам за месяц. Факт, а не
     * план, поэтому не амортизируются. Вычтены из netProfit. Optional: старый
     * бэкенд = 0.
     */
    salaryExtras?: number;
  ...
  projection: {
    ...
    recurringExcess: number;
    /** Run-rate премий и «мотивации» (ongoing, а не сунк). Optional: старый бэкенд = 0. */
    salaryExtras?: number;
    netProfit: number;
  };
...
export interface SalaryPayout {
  ...
  periodMonth?: string | null;
  /**
   * 161 — филиал, за счёт которого выплата сделана (текущая точка владельца в
   * момент выдачи). Зеркальный расход получает ЭТУ ЖЕ точку, поэтому
   * «начислено − выплачено» на филиале всегда сходится: начисления берутся по
   * филиалу ЧЕКА, выплаты — по этой колонке. null = одноточечный тенант.
   */
  pointId?: string | null;
  /**
