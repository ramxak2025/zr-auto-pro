...
Disclosed reference for [`expo-web-to-native`](../SKILL.md). A _false friend_ is a web idiom that looks portable but isn't - it compiles in your head, then breaks (or silently no-ops) on native. Each row is `web idiom → native equivalent → the gotcha`. Reach for this in steps 3–5 of a migration, and whenever you're learning native after the web.
...
| `useParams` / `useSearchParams` | `useLocalSearchParams()`         | Note: doesn't work _inside_ a DOM component - read in the native parent, pass as props (see `expo-dom`).                                |
