...
Disclosed reference for [`expo-web-to-native`](../SKILL.md), step 4. `false-friends.md` translates _idioms_ (`div` → `View`); this translates _UX patterns_ - a web interaction into its native equivalent. Step 4 isn't a port, it's this redesign.
...
The table gets the right _components_; "native" also lives in _motion and touch_ - and a web port arrives with almost none, so you must add it. The tooling is in `expo-router` (navigation/transitions) and `expo-native-ui` (motion/effects); reach for it, don't hand-roll:
