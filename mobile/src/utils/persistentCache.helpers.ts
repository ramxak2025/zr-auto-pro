...
export const PERSISTED_KEYS = [
  ...
  'marketing-report',
  // ── Филиалы (мульти-точки 156/160) ────────────────────────────────────
  // ['points'] — единственный фиксированный слот: живые точки тенанта + моя
  // текущая точка. Персистим, чтобы на холодном старте пункт «Филиалы» в
  // «Ещё» и индикатор филиала в Кассе/Журнале появлялись СРАЗУ, а не через
  // такт сети: без имени точки индикатор нечем нарисовать, а его отсутствие
  // читается как «филиал не важен». Денег в слоте нет — только справочник,
  // поэтому его безопасно поднимать до первого ответа сервера. При смене
  // филиала весь персистентный кеш чистится целиком (см. usePoints).
  'points',
] as const;
