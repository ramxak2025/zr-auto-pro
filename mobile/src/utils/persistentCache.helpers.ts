...
export const PERSISTED_KEYS = [
  ...
  // «Ещё» и индикатор филиала в Кассе/Журнале появлялись СРАЗУ, а не через
  // такт сети: без имени филиала индикатор нечем нарисовать, а его отсутствие
  // читается как «филиал не важен». Денег в слоте нет — только справочник,
  // поэтому его безопасно поднимать до первого ответа сервера. Любая смена
  // филиала — и вход заново (163), и мгновенный перевыпуск сессии
  // руководителем (167) — проходит через один и тот же коммит сессии, а он
  // чистит весь диск целиком (см. clearPersistentCache), поэтому филиал
  // прошлой сессии отсюда не поднимется.
  'points',
] as const;
