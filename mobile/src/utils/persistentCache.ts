...
/**
 * Сброс персистентного кеша при СМЕНЕ ФИЛИАЛА (мульти-точки 156/160).
 *
 * Слоты на диске пишутся БЕЗ сегмента филиала: ключ слота — это query-key, а
 * филиал в него не входит (он живёт в JWT-акторе на сервере). Значит после
 * переключения точки любой персистированный слот — снимок ПРОШЛОГО филиала, и
 * на следующем холодном старте он поднялся бы обратно в экран как «свежие»
 * деньги. Сегментировать сотню ключей филиалом мы сознательно не стали:
 * пропустить один ключ = показать владельцу чужую выручку, а цена полной
 * очистки — всего одна медленная загрузка после редкого ручного действия.
 *
 * Семантически это тот же барьер, что и на логауте (та же генерация,
 * отменяющая отложенные записи прошлого владельца), поэтому реализация одна.
 */
export function clearPersistentCacheForPointSwitch(): Promise<void> {
