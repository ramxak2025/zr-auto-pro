 ...
 *
 * ЭТОТ ХУК ФИЛИАЛ НЕ МЕНЯЕТ (163/167). Филиал — свойство СЕССИИ: он лежит в
 * подписанном токене, и сменить его можно только выдав НОВУЮ сессию. Меняется
 * он ровно в одном месте приложения — в разделе «Филиалы» (требование
 * владельца «переключаться только там»), и ровно двумя способами:
 *   • руководитель (право user_management) — мгновенным перевыпуском сессии
 *     (POST /auth/switch-point, 167: новый токен, старый гасится);
 *   • остальные сотрудники — выходом и повторным входом с выбором филиала
 *     (163). Для них ничего не изменилось.
 * Поэтому здесь по-прежнему НЕТ ни ручки переключения, ни подстановки филиала:
 *   • POST /points/switch клиент не зовёт вовсе (сервер оставил её ради сборок
 *     3.5/3.6 — она лишь запоминает филиал для СЛЕДУЮЩЕГО входа);
 ...
 * ничего не переключает (163): филиал сессии меняет только перевыпуск токена.
 */
...
export interface PointAccess {
   ...
   * Основной сервис первым. Как именно он туда попадёт — решает раздел
   * «Филиалы»: руководителю сессия перевыпускается мгновенно (167), остальным
   * по-прежнему нужен выход и повторный вход (163).
   */
