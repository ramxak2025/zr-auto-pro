...
## Iteration #6 — 2026-06-19 (нативное Liquid-Glass стекло под фильтр-чипами Журнала)

Журнал (вкладка «Склад. документы») получил нативную Liquid-Glass подложку под рядом kind-чипов (Все/Покупки/Возврат клиента/Возврат поставщику/Брак/Списания/Платежи/Б/У). Выбран самый низкорисковый путь: нативный **материал** (`AutexaGlassHeaderView.swift` — `UIVisualEffectView` + `systemThinMaterial`, авто-апгрейд до iOS 26 `UIGlassEffect` через `NSClassFromString`) как фон, а сами чипы остаются RN — источник истины для kind/label/цвета фильтра (только что пофикшенного) и драйвят тот же `warehouseKind` state. Гарантированный fallback: JS-обёртка `AutexaGlassHeader.tsx` рендерит нативное стекло только на iOS с присутствующим модулем, иначе прозрачный passthrough `View` — Android и отсутствие модуля рендерят те же чипы byte-for-byte. Новый модуль `AutexaGlassHeaderModule` зарегистрирован в `expo-module.config.json`; автолинкинг подтверждён в `ExpoModulesProvider.swift`. Детали, fallback-таблица и acceptance: `docs/ios-redesign/JOURNAL_GLASS_HEADER_NATIVE.md`.

Проверки: mobile tsc/eslint — зелёные; jest 276/276 — зелёные; `expo prebuild --clean` + `pod install` (авто) + `xcodebuild` (Debug, iphonesimulator, compile-only, `CODE_SIGNING_ALLOWED=NO`) — **BUILD SUCCEEDED**, `AutexaGlassHeaderView.swift` скомпилирован под x86_64+arm64, ноль ошибок компилятора. На устройстве не запускалось (compile-only по флоу владельца) — визуал и iOS 26 glass владелец проверяет на физическом iPhone.

## Iteration #7 — 2026-07-18 (интеграция API-кольца PR #10 + ревью-фиксы + TestFlight 3.0.2)

Интегрирован PR #10 (Codex): кольцо из трёх API-хостов (autexa.pw → autexa-cloud.ru → Яндекс-шлюз), последовательный логин с общим бюджетом 30с, «окончательные» ошибки (400/401/422/429/500) не размножаются по алиасам, ре-выбор маршрута на смене сети/VPN/foreground, epoch-guard от устаревших auth-ответов, безопасное восстановление сессии, разделение кэша между пользователями, backend: /health вне rate-limit + снятие дублирующего guard'а на login/register. Интеграция — fast-forward; adversarial-ревью нашло и закрыло 3 дефекта исходного кода PR (гидрация persistentCache, ложное «сохранён» для отменённых чеков, деструктивный сброс кэша при сбое чтения) — детали в `docs/API_RING_MOBILE_ACCEPTANCE.md`.

Проверки: mobile tsc/eslint/jest 515/515 — зелёные; backend tsc/lint/build + node-тесты 4/4 — зелёные; frontend tsc/build — зелёные; CI GitHub — зелёный. Backend задеплоен (rolling, health-gated), /health ×3 — 200. TestFlight: **3.0.2 (70)**, внутренние тестировщики. На устройстве не запускалось — приёмка владельцем по матрице `docs/API_RING_MOBILE_ACCEPTANCE.md` (моб. сеть / Wi-Fi / VPN, отказ каждого хоста, сохранённый старый хост, восстановление после смены сети).

## Iteration #8 — 2026-07-27 (Round 12: ночной автономный батч из 9 пунктов, v3.2.0)

Запрос владельца (надиктован ночью, реализован автономно до утра): журнал мерцает блоками + плохой фильтр + дизайн-проход; госномер переставляет цифры при быстром наборе; клиент без телефона с подтверждением; права role-only без старых тумблеров; тап по товару в чеке → карточка склада; сканер штрихкодов (формы товара + мгновенное добавление в Кассе); confirm «розничный покупатель»; порядок в Маркетинге с видимыми анти-спам-гарантиями; метки чеков + аналитика по меткам.

Сделано тремя волнами узких агентов (5+3+2) по непересекающимся файлам с разведкой-предварительно (9 read-only карт с подтверждёнными корнями). Ключевые корни: микросекундная дыра keyset-курсора журнала (пачки чеков одной транзакции выпадали блоком), async-коммит контролируемого selection-пропа на Fabric (перестановка цифр номера), физически неработавшие тумблеры видимости (auth/me их никогда не отдавал), невидимость серверных анти-спам-гарантий маркетинга. Миграции 140 (метки, RLS) и 141 (безопасные дефолты рассылок, без backfill). Полные детали и acceptance-чеклисты: `docs/ios-redesign/ROUND12_BATCH.md`, `docs/ios-redesign/MARKETING_REDESIGN_V2.md`.

Проверки: backend/frontend/mobile typecheck+lint+build — зелёные; mobile jest 628/628 (plateMask 38/38); QA-гейт с prebuild+pod+xcodebuild — см. вердикт в отчёте сессии. На устройстве не запускалось — приёмка владельцем на физическом iPhone по чек-листам ROUND12_BATCH.md.

## Iteration #9 — 2026-07-30 (Round 13: рассрочки, v3.2.1)

Батч владельца из 9 пунктов по рассрочкам: «скидка не применяется» (оказалось — семантика «скидка только на товары», решением владельца сохранена + честные предупреждения в Кассе), бейдж «installment» → «Рассрочка», редизайн карточки рассрочки (обрезанный «Остаток», клиент+авто с кнопками позвонить/WhatsApp, фото из чека, поручители — мигр. 142, перенос даты с причиной и историей — мигр. 143, звонки клиента с плеером записей), разблокировано редактирование чека рассрочки с транзакционным пересчётом плана. Денежный контур прошёл adversarial-ревью (3 линзы, 8 находок → 3 дефекта закрыты до коммита: дрейф должника, разрушительный кап взноса, ложная «просрочка» при реоткрытии). Попутно закрыт guard в calls (клиент без телефона: 500/утечка всех звонков тенанта). Детали: `docs/ios-redesign/ROUND13_INSTALLMENTS.md`.

Проверки: backend/frontend/mobile typecheck+lint+build зелёные, jest 628/628, QA-гейт с prebuild+pod+xcodebuild — вердикт в отчёте сессии. App Store: 3.2.0 (Round 12) одобрена и READY_FOR_SALE. На устройстве не запускалось — приёмка владельцем по чек-листам.

## Iteration #10 — 2026-08-03 (Round 14, зона 5: доставка уведомлений + гибкие настройки)

Жалоба владельца: «нативных уведомлений нет, почини и чтобы гибко настраивать». Корень найден и подтверждён **на сгенерированном артефакте**: `mobile/app.json` объявлял плагин `expo-notifications` без `mode`, а плагин по умолчанию пишет `aps-environment: development` — то есть каждая production/TestFlight-сборка регистрировалась в **sandbox** APNs, тогда как Expo доставляет в **production** APNs. Баг был невидим, потому что Expo-тикет возвращает `ok` (тикет = «принято в очередь»), реальный отказ APNs живёт только в delivery receipt, а receipts бэкенд никогда не читал; плюс `logger.error` не доходил до Sentry (`integrations: []`). Побочно вскрылось, что в `Info.plist` вообще не было `UIBackgroundModes`, поэтому тихие data-пуши инвалидации кеша тоже никогда не будили приложение.

Починка: новый `mobile/app.config.js` вычисляет `aps-environment` (`APS_ENV` → `EAS_BUILD_PROFILE` → **дефолт `production`**, чтобы поломка переменных давала громкий отказ подписи dev-сборки, а не тихую потерю пушей у пользователей) + `enableBackgroundRemoteNotifications`; чтение delivery receipts с прунингом мёртвых токенов и явным `Sentry.captureMessage`; `Authorization: Bearer EXPO_ACCESS_TOKEN` (страховка от включения Enhanced Security); честный `{ registered: false }` вместо молчаливого 200 при срабатывании anti-hijack-гарда. Гибкость: миграция 151 (`notification_settings`: мастер-тумблер, тихие часы в локальном времени пользователя, звук), категории расширены 5 → 11, шесть пушей, слав­шихся вообще без тумблера (готовность, оплата, записи, звонки, профиль, аккаунт), переведены на `sendToUserCategory`; экран «Уведомления» пересобран по секциям + блок диагностики с тестовым пушем, возвращающим сырой вердикт Expo. Детали, грабли prebuild и acceptance-чеклист: `docs/ios-redesign/PUSH_NOTIFICATIONS_FIX.md`.

Проверки: backend typecheck+lint+build и mobile/frontend typecheck+lint+build — зелёные; backend node:test 26/26 (9 новых), mobile jest 636/636 (8 новых, plateMask 38/38). Дополнительно: `expo prebuild --ios --clean` подтвердил `aps-environment: production` и `UIBackgroundModes: [remote-notification]`; миграция 151 и оба новых SQL-запроса прогнаны на живом PostgreSQL 16 (идемпотентность, дефолты без строки, upsert, CHECK, CASCADE). **Доставка на физический iPhone не проверялась — нужна новая сборка (OTA нативный entitlement не доставляет); приёмка владельцем по чек-листу PUSH_NOTIFICATIONS_FIX.md §8.** Android-пуши остаются невозможны до заведения Firebase-проекта владельцем.

## Iteration #10 — 2026-08-03 (Round 14: кассир, деньги по периодам, уведомления, v3.3.0)

Крупнейший батч: пять зон, 8 миграций (144-151), 10 коммитов. (1) Sentry-находки: logout-500 у tenantless (токен не отзывался), push-шум Expo, принудительные разлогины → одноразовый refresh с ревокацией линии. (2) Поставщики: сторно платежа с причиной и компенсацией баланса + «Возврат от поставщика», под новой ячейкой матрицы (по умолчанию только Директор). (3) Режим «Кассир» доведён из сырого: конвейер Принят→В работе→Готова→Оплачена→Выдана, несколько исполнителей, справочник мест, per-role таб-бар (без правок Swift), узкий роут приёма оплаты, гард смены режима, push-цепочка; спека — `CASHIER_MODE_SPEC.md`. (4) Деньги по периодам: «за какой месяц» у выплат и платежей, история ставок мастера по месяцам с точечным пересчётом, выплаты вне программы. (5) КОРЕНЬ «уведомления не приходят»: плагин без `mode` → `aps-environment: development` в релизе (sandbox APNs при отправке в production, тикет `ok`, квитанции не читались) — исправлено + диагностика в UI и гибкие настройки (мастер-тумблер, тихие часы, 6 новых категорий).

Adversarial-ревью в 5 линз: 19 находок (1 critical — кассир по пресету не мог принять оплату; 3 high в auth-refresh; high-регрессия старых сборок), все закрыты до выкатки. Детали и чек-листы: `docs/ios-redesign/ROUND14_BATCH.md`.

Проверки: backend/frontend/mobile typecheck+lint+build зелёные, mobile jest 636/636, backend node:test 30/30, QA-гейт с prebuild+pod+xcodebuild и проверкой энтайтлментов — вердикт в отчёте сессии. Требует владельца: Firebase для Android-пушей, APNs-ключ в EAS.

## Iteration #11 — 2026-08-03 (Round 15: зарплатные корректировки и помесячные отчёты, v3.3.1)

Корректирующий батч по фидбеку владельца на 3.3.0: (1) процент мастера меняется прямо из карточки месяца в Зарплате (mobile+web) с пересчётом начислений И прибыли только этого месяца; (2) владелец отменяет/правит ошибочные выплаты и штрафы (мигр. 153: сторно расхода, аудит-след, зачёркнутые строки, всё транзакционно с FOR UPDATE OF); (3) финансовый отчёт за любой месяц — пейджер «‹ Июль 2026 ›» (попутно закрыт разрыв: пресет «Месяц» не покрывал месяц целиком и period_month-расходы не попадали в отчёт). Два adversarial-ревью: 1 medium по деньгам (матч расхода мог удалить чужой — сужен по имени) + 2 medium по швам с клиентами (бессмертные модалы) + 5 low — все закрыты до релиза; бонусом вылечен давний цикл confirm-модала (getAll не отдавал confirmedAt). Backend 70/70, mobile 636/636, гейт с prebuild+xcodebuild зелёный, aps-environment=production подтверждён. Детали: ROUND15 в памяти проекта; живая приёмка — на тестовом тенанте.

## Iteration #12 — 2026-08-21 (Round 16: кассовая смена v2 — сейф и кассиры; 4 точечные правки)

Августовский список владельца, 5 пунктов. (1) Инвентаризация: чипы категорий растягивались капсулами на пол-экрана (ScrollView без `flexGrow: 0` конкурировал с FlashList) — однострочный фикс, экран снова рабочий. (2) Склад: кнопка «Переместить (N)» в массовом выделении (новый транзакционный `POST /products/bulk-move`) + перенос папки в другую папку/подпапку через ужесточённый `rename` (warehouse-скоуп, запрет цикла, коллизия → 400). (3) Записи: комментарий редактируется/добавляется после создания (UI-only, контракт уже был). (4) Поставщики: таб-бар больше не перекрывает поставки; корректировка поставки (пересчёт остатков корректирующими stock_movements + долг дельтой, блок при несторнированном платеже); удаление = пометка «Удалена» с откатом остатков (мигр. 154). (5) Кассовая смена v2 (мигр. 155): настройка «Кто принимает оплату» (allowlist поверх ролей, гард включения режима без кассира), атрибуция `accepted_by` («деньги видны на принявшем» в движении денег и Z-отчёте), сейф как отдельный кошелёк + размен-пересменка (закрытие: часть в сейф, остаток — старт следующей смены), сдача по сотрудникам при закрытии, инкассация из кассы/сейфа без подтверждения кассира (кассиру пуш), пуш владельцу «Касса закрыта» (2 новые категории уведомлений), рассрочка при приёме оплаты кассиром, тонировка журнала по оплате, доска: тап → заказ, лонг-пресс → этап, быстрый комментарий, фото исполнителей 32pt, «Выдана» заблокирована до оплаты. Админ не из списка автоматически получает приёмку «Отправить на доску» без блока оплаты. Детали: `CASH_SHIFT_V2_SAFE_2026_08_21.md`, `FIXES_ROUND_2026_08_21.md`.

Проверки: backend typecheck+lint+build, frontend typecheck+build, mobile typecheck+lint+jest 636/636 — зелёные. Физический iPhone в сессии недоступен — приёмка по чек-листам в двух markdown выше.

## Iteration #13 — 2026-09-13 (3.7.2: отметка прихода снова работает; филиал у графика, плана постоянки и записей)

Жалоба владельца: «перестало работать расписание — невозможно отметить, кто пришёл, кто опоздал» и «при переключении филиалов данные с основной точки: расписание, постоянные расходы и так далее — это отдельные автосервисы». Два корня. (1) `ScheduleService.ensureShiftOpen` (3.7.0) считал филиал автооткрытой смены агрегатом `MIN(tp.id)` по uuid — в PostgreSQL 16 `min(uuid)` не существует (воспроизведено на 16.14), каждая отметка падала с 500 после записи строки графика, клиент откатывал её. Теперь филиал смены = филиал строки графика, параметром. (2) Графику, плану постоянных расходов и записям волна 161 не дала своего `point_id`, выразив филиал составом команды; по дефолту 156 сотрудник без назначений виден везде, и у тенанта, не расставившего людей по филиалам, оба автосервиса показывали одно и то же. Миграция 167: `point_id` у `schedule_entries` / `fixed_costs` / `bookings`, привязка истории (график — по единственному живому назначению, иначе основной; записи — по чеку → назначению мастера → основной; план — основному), та же лестница в `PointsService.HISTORY_ATTACH_SQL`. Скоуп строгим равенством: сетка, «кто на работе», зарплата «за смену», карточка сотрудника, алерты, план и accrual-прибыль филиала, записи; день переезжает в филиал, который его последним записал (один день на сотрудника во всей сети). Клиенты: строки сетки и пикер окладов — команда филиала (`?scope=point`, ключ `['users','point']`), подпись автосервиса на «Постоянных расходах». Детали и приёмка: `POINT_SCHEDULE_PLANNING_2026-09-13.md`.

Проверки: backend build + 379 node-тестов + tsc + lint; frontend tsc + lint + build; mobile tsc + lint + jest 757/757. Живой PostgreSQL 16.14: миграции 028–167 прогнаны настоящим `MigrationRunner`, 167 повторно (идемпотентна), сеющий скрипт с ROLLBACK подтвердил привязку, перенос дня, смену в филиале строки, «кто на работе» в обоих автосервисах и гейт записи. Native-код не менялся; сборки — EAS iOS 3.7.2 (87) с автоотправкой в TestFlight и Android preview APK (99). На физическом iPhone в сессии не запускалось — приёмка владельцем по чек-листу в документе выше.

## Iteration #14 — 2026-09-14 (3.8.0: филиал — отдельный автосервис: свой склад, штат, имущество)

Владелец: «у каждой точки свой товар и остатки, свои мастера и так далее… каждый филиал — отдельный автосервис под одним владельцем и одной подпиской». После 3.7.2 склад оставался один на тенант (три склада с ключом по тенанту), штат по умолчанию общим (сотрудник без назначений виден везде), имущество без филиала. Миграция 168: `storage_items.point_id`; каждый не-владелец приписан к филиалу (по последнему чеку → смене → основной сервис), списки сотрудников по умолчанию — штат филиала (`?scope=all` — сеть), новый сотрудник — к филиалу создателя. Миграция 169: `warehouses.point_id`, ключ `(tenant_id, point_id, kind) NULLS NOT DISTINCT`, существующие склады — основному сервису, каждому филиалу — пустой набор из трёх; `deliveries` / `purchase_orders` / `supplier_payments` — свой филиал; новый тип движения `point_transfer` (перенос товара в другой филиал, только руководителю). На сервере филиалом режутся товары, папки, движения, поставки, заказы поставщикам, оплаты, возвраты, журнал складских документов, складская аналитика, отчёт по браку, алерты, подсобка; чек списывает остаток только товара своего филиала. Мобилка: перенос в филиал в меню товара, откат устаревшего склада в Кассе/аналитике после мгновенной смены филиала, подписи автосервиса на складских экранах. Web: папки по выбранному складу. Детали и приёмка: `POINT_WAREHOUSE_STAFF_2026-09-14.md`.

Проверки: backend build + 379 node-тестов + tsc + lint; frontend tsc + lint + build; mobile tsc + lint + jest 757/757. Живой PostgreSQL 16.14: 168 и 169 прогнаны раннером и повторно (идемпотентны), сеющие скрипты с ROLLBACK подтвердили приписку штата, привязку имущества, три склада на филиал без сирот, изоляцию товара, резолв основного склада, перенос, отказ списания чужим чеком, привязку поставок и оплат. Native не менялся; сборки — EAS iOS 3.8.0 (TestFlight, внешняя группа «Autexa beta») и Android preview APK. На устройстве в сессии не запускалось — приёмка владельцем по чек-листу.
