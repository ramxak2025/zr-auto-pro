...
test('MEDIUM: старый разворот в целые месяцы удалён (getMonthYearsForRange + IN)', () => {
  assert.doesNotMatch(
    service,
    /getMonthYearsForRange\(/,
    'метод разворота диапазона (его вызов/определение) должен быть удалён',
  );
...
test('membership: месяц-к-дате (clamp LEAST к сегодня) + fallback по дате факта', () => {
  ...
  assert.match(
    body,
    /now\(\) AT TIME ZONE \$\{tzPh\}/,
    'сегодня — в бизнес-таймзоне ТЕНАНТА, и пояс приходит ПЛЕЙСХОЛДЕРОМ (склейка = инъекция)',
  );
  assert.doesNotMatch(body, /AT TIME ZONE '/, 'литерал пояса в SQL недопустим — только параметр');
...
test(
  ...
  {
    skip: !process.env.SALARY_LIVE_DB
      ? 'нет SALARY_LIVE_DB (throwaway-Postgres) — статические стражи покрывают структуру'
      : false,
  },
  async () => {
    ...
    try {
      ...
      const paidMonth = async (ym) =>
        Number(
          (
            await c.query(
              `SELECT COALESCE(SUM(amount),0) s FROM salary_payouts WHERE employee_id=$2 AND status='accepted' AND COALESCE(period_month, to_char(created_at AT TIME ZONE '${TZ}','YYYY-MM'))=$1`,
              [ym, EMP],
            )
          ).rows[0].s,
        );
      ...
      const now = (
        await c.query(
          `SELECT to_char(now() AT TIME ZONE '${TZ}','YYYY-MM') ym, to_char(now() AT TIME ZONE '${TZ}','YYYY-MM-DD') today`,
        )
      ).rows[0];
      ...
      await c.query(
        `INSERT INTO salary_payouts(employee_id,amount,status,period_month) VALUES ($1,40000,'accepted',$2)`,
        [EMP, now.ym],
      );
      ...
      assert.equal(
        await paid(`${now.ym}-01`, curLast),
        await paidMonth(now.ym),
        'getAll(month) == getEmployeeMonth(month)',
      );
      await c.query(
        `INSERT INTO salary_payouts(employee_id,amount,status,period_month) VALUES ($1,15000,'cancelled',$2)`,
        [EMP, now.ym],
      );
      ...
      await c.query(
        `INSERT INTO salary_premiums(user_id,type,amount,period_month_year) VALUES ($1,'cash',9000,'2020-07'),($1,'cash',8000,'2020-08')`,
        [EMP],
      );
