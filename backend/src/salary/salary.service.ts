...
export class SalaryService {
  ...
  async getAll(tenantID: string, query: any) {
    ...
    const { rows } = await this.pool.query(
      `SELECT u.id as master_id, u.full_name as master_name,
              ...
              COALESCE(SUM(ch.service_salary_total), 0) as total_earnings,
  ...
  async getMy(tenantID: string, userID: string) {
    ...
    const { rows: userRows } = await this.pool.query(
      'SELECT full_name, COALESCE(salary_percent, 0) as salary_percent FROM users WHERE id=$1 AND tenant_id=$2',
      [userID, tenantID],
    );
    const user = userRows[0] || { full_name: '', salary_percent: 0 };

    const { rows } = await this.pool.query(
      `SELECT
         COALESCE(SUM(CASE WHEN date >= $2 THEN service_salary_total END), 0) as today,
         COALESCE(SUM(CASE WHEN date >= $3 THEN service_salary_total END), 0) as week,
         COALESCE(SUM(CASE WHEN date >= $4 THEN service_salary_total END), 0) as month,
         COALESCE(SUM(service_salary_total), 0) as total,
