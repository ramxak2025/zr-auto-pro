 ...
 *
 * Config (settings) is owner-class only (director/admin/superadmin). The pass
 * download is open to ANY authenticated tenant user (a cashier may hand a client
 * their card) but is tenant-scoped — it only ever generates a pass for this
 * tenant's client. The cert/key/password/WWDR are WRITE-ONLY — getSettings returns
 * only boolean "stored" flags, never the raw PEM.
 *
