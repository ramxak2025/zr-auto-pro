...
export class TenantsService {
   ...
   *
   * ФИЛИАЛА (claim `pointId`, 163) здесь СОЗНАТЕЛЬНО НЕТ: суперадмин входит в
   * чужой кабинет и не может знать, в каком филиале владелец сейчас работает.
   * JwtStrategy подставит такой сессии филиал по умолчанию (последний
   * использованный владельцем, иначе основной сервис) — та же ветка, что у
   * старых сборок. Это ровно то, что нужно от 30-минутного визита: работать в
   * КОНКРЕТНОМ филиале, а не «во всех сразу».
   *
   * ВХОД ПОД ВЛАДЕЛЬЦЕМ РАБОТАЕТ И ПОСЛЕ ЗАКРЫТИЯ ЕГО ФИЛИАЛА (166). Раньше
   * этот токен был бесполезен ровно в тот момент, когда нужнее всего: владелец
   * назначен только на филиал, суперадмин филиал заархивировал — и
   * autexa_default_point не находил НИ ОДНОГО доступного филиала, так что
   * первый же запрос такой сессии получал 401. Суперадмин не мог войти
   * и исправить назначения владельца, а сам владелец — тем более. Теперь
   * держателю права управления персоналом (а владелец-директор им является по
   * роли) autexa_available_points отдаёт основной сервис, и подстановка ниже
