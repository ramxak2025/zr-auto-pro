...
export class ChecksService {
  ...
  async getAll(tenantID: string, query: any, actor?: ChecksActor) {
    ...
    if (query.masterId) {
      // СТРОГО по главному мастеру чека. Решение владельца 2026-07-28: фильтр
      // «по сотруднику» везде означает «ЕГО чеки» (ch.master_id) —
      // консистентно с «Движением денег» (reports.getCashflow считает деньги
      // мастера тем же строгим предикатом; drill-down дня из cashflow шлёт
      // сюда ?masterId и обязан получить ровно те чеки, что вошли в сумму).
      // Round 12 временно расширял фильтр executor-OR'ом (c70c094) — откачено:
      // исполнительство строк — про ВИДИМОСТЬ (предикат checks_view_all выше),
      // не про принадлежность чека.
