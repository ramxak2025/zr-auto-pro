...
export class PlansService {
  ...
  async create(dto: any) {
    const { rows } = await this.pool.query(
      ...
      [dto.name, dto.monthlyPrice || 0, dto.description, JSON.stringify(dto.features || []),
       dto.maxUsers || 5, dto.sortOrder || 0],
    );
  ...
  async update(id: string, dto: any) {
    ...
    if (dto.features !== undefined) { sets.push(`features=$${idx++}`); vals.push(JSON.stringify(dto.features)); }
    ...
    if (dto.sortOrder !== undefined) { sets.push(`sort_order=$${idx++}`); vals.push(dto.sortOrder); }
